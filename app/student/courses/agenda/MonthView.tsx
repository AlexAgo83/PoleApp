  ...
  return (
    <section className={containerClass}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-slate-200 ml-auto">
          <MonthNav
            prev={prev}
      ...
      <div className="mt-3">
        <div className="grid grid-cols-2 gap-1.5 text-sm text-slate-200 sm:grid-cols-3 sm:gap-2 md:grid-cols-4 md:gap-3 lg:grid-cols-7">
          {cells.map((cell, idx) => {
            ...
            const cellDate = cell.isoDate ? new Date(cell.isoDate) : cell.day ? new Date(`${month}-${String(cell.day).padStart(2, "0")}T00:00:00`) : null;
                ...
                {cell.courses &&
                  cell.courses.slice(0, 3).map((course) => {
                    ...
                    return (
                      <Link
                        key={course.id}
                        href={`/student/courses/${course.courseId}?from=${encodeURIComponent(baseFrom)}`}
                        ...
                        style={
                          course.photoPublicId && CLOUD_NAME
                            ? {
                                backgroundImage: `linear-gradient(135deg, rgba(86,123,255,0.24), rgba(196,88,255,0.2)), linear-gradient(135deg, rgba(10,15,30,0.06), rgba(15,25,45,0.05)), url(https://res.cloudinary.com/${CLOUD_NAME}/image/upload/c_fill,g_auto,f_auto,q_auto,e_blur:1600,w_600,h_360/${course.photoPublicId})`,
                      ...
                      >
                        <div
                          className="flex-1 space-y-0.5 overflow-hidden pr-6"
                          style={{ textShadow: "0 2px 14px rgba(0,0,0,0.75)" }}
                        >
                          <p className="whitespace-nowrap text-[9px] text-cyan-100">
