...
export default async function StudentCoursesAgendaPage({
  ...
  const buildViewHref = (mode: "month" | "week") => {
    ...
    if (q) params.set("q", q);
    return `/student/courses/agenda${params.toString() ? `?${params}` : ""}`;
  };
  ...
  const legendItems = [
    ...
    { key: "attending", label: "Inscrit (à venir)", className: "border border-emerald-300/70 bg-emerald-500/20 text-emerald-50" },
      ...
      <section className="panel space-y-4 border-indigo-400/25 p-6 shadow-indigo-900/30">
        ...
        {view === "month" && (
          <MonthView
            ...
            baseFrom="/student/courses/agenda"
          />
        )}
