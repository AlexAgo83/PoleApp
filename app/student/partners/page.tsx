...
export default async function StudentPartnersPage() {
  const session = await getServerSession(authOptions);
  if (!session?.user?.schoolId || session.user.role !== "STUDENT") {
    return (
      <main className="flex min-h-screen w-full flex-col gap-4 p-4 md:p-6">
        <section className="panel p-6">
          ...
          <p className="text-slate-300">Accès réservé aux élèves rattachés à une école.</p>
          <Link
            href="/student"
