...
export default async function StudentTeachersPage({
  ...
  if (!session?.user?.id) {
    redirect("/login?callbackUrl=/student/teachers");
  }
  ...
  const prevHref = `/student/teachers?page=${Math.max(1, safePage - 1)}${qs ? `&${qs}` : ""}`;
  const nextHref = `/student/teachers?page=${Math.min(totalPages, safePage + 1)}${
    qs ? `&${qs}` : ""
  }`;
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div className="flex w-full items-center sm:w-auto sm:justify-end">
                      <Link
                        href={`/teachers/${teacher.id}?from=/student/teachers`}
