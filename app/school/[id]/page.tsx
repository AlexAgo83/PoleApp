...
import { WeekView as StudentWeekView } from "@/app/student/courses/agenda/WeekView";
import { WeekView as TeacherWeekView } from "@/app/teacher/courses/agenda/WeekView";
...
const LEGEND_ITEMS = [
  ...
  { label: "Inscrit (à venir)", className: "border border-emerald-300/70 bg-emerald-500/20 text-emerald-50" },
...
export default async function StudioPage({ params, searchParams }: PageProps) {
  ...
  const listHref = `/school/${studioId}${baseParams.toString() ? `?${baseParams.toString()}` : ""}`;
  ...
  const agendaHref = `/school/${studioId}?${agendaParams.toString()}`;
  ...
  const weekRangeHref = `/school/${studioId}?${weekRangeParams.toString()}`;
  ...
  const monthRangeHref = `/school/${studioId}?${monthRangeParams.toString()}`;
  ...
  const agendaBaseFrom = `/school/${studioId}?${agendaBaseFromParams.toString()}`;
  const monthCells =
    agendaRange === "month"
      ? [
    ...
    <main className="flex min-h-screen w-full flex-col gap-4">
      <header className="panel relative overflow-hidden p-4 md:p-6 space-y-3" style={headerBgStyle}>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 text-sm">
          ...
          {isAdmin && (
            <Link
              href={`/admin/school?edit=${studio.id}`}
        ...
        <section className="panel p-4 md:p-6">
                        ...
                        <Link
                          href={`${
                            userRole === "STUDENT"
                              ? "/student/courses"
                              : "/teacher/courses"
                          }/${course.id}?from=/school/${studio.id}`}
          ...
          {studio.courses && studio.courses.length > 0 && totalPages > 1 && (
            <div className="mt-4 flex items-center justify-between text-sm text-slate-200">
              <Link
                href={`/school/${studioId}?page=${Math.max(1, currentPage - 1)}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
              ...
              <Link
                href={`/school/${studioId}?page=${Math.min(totalPages, currentPage + 1)}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
                className={`rounded-full px-3 py-2 font-semibold ${
