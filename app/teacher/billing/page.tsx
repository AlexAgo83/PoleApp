      ...
      <section className="panel space-y-4 p-4 md:p-6">
          ...
          {invoices.map((invoice) => {
            ...
            return (
              <article key={invoice.id} className="flex flex-col gap-2 py-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Link
                    href={`/teacher/courses/${course.id}?from=/teacher/billing`}
        ...
        {totalCount > 10 && (
          <div className="mt-6 flex items-center justify-between">
            <Link
              href={`/teacher/billing?page=${Math.max(1, currentPage - 1)}${qs ? `&${qs}` : ""}`}
            ...
            <Link
              href={`/teacher/billing?page=${Math.min(totalPages, currentPage + 1)}${qs ? `&${qs}` : ""}`}
              className={`rounded-full px-3 py-2 text-sm font-semibold ${
