  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (res.status === 401) {
          router.replace(`/login?from=${encodeURIComponent(safeFrom || "/teacher/courses/agenda?view=month")}`);
          return;
        }
        if (res.status === 403) {
      ...
      <main className="flex min-h-screen w-full flex-col gap-4">
        <div className="panel border-red-400/25 bg-slate-900/70 p-6 text-white shadow-red-900/40 md:p-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <button
              type="button"
              onClick={() => router.replace(safeFrom || "/teacher/courses/agenda?view=month")}
