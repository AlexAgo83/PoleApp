...
export default async function TeacherSchoolPage({
  ...
  const studioFilters =
    typeof params.studio === "string"
      ? params.studio
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean)
      : Array.isArray(params.studio)
      ? params.studio.flatMap((s) => s.split(",")).map((s) => s.trim()).filter(Boolean)
      : [];
  const teacherFilter =
  ...
  if (disciplineFilters.length) paramsForLinks.set("discipline", disciplineFilters.join(","));
  ...
  const buildViewHref = (mode: "week" | "month") => {
    ...
    return `/app/teacher/school${nextParams.toString() ? `?${nextParams}` : ""}`;
  };
  ...
  const baseFrom = `/app/teacher/school${baseFromParams.toString() ? `?${baseFromParams.toString()}` : ""}`;
  const agendaHref = `/app/teacher/courses/agenda${paramsForLinks.toString() ? `?${paramsForLinks}` : ""}`;
  const listHref = `/app/teacher/courses${paramsForLinks.toString() ? `?${paramsForLinks}` : ""}`;
    ...
    <main className="flex min-h-screen w-full flex-col gap-4">
      ...
      <section className="panel space-y-4 p-6">
        ...
        >
          <form
            method="get"
            key={`teacher-school-agenda-${monthParam ?? "current"}`}
          ...
          >
            <label className="text-sm text-slate-200">
              ...
              <input
                type="month"
                name="month"
                defaultValue={monthParam ?? monthValue}
          ...
          {sortedUpcoming.map(({ course, myAttendance }) => {
            ...
            return (
              <article key={course.id} className={`flex flex-wrap items-start gap-3 py-3 ${isPast ? "opacity-75" : ""}`}>
                ...
                <Link
                  href={`/app/teacher/courses/${course.id}?from=/app/teacher/school`}
