    ...
    <main className="flex min-h-screen w-full flex-col gap-4">
      ...
      <section className="panel space-y-4 p-6">
              ...
              <div className="flex w-full flex-col items-start gap-2 text-sm">
                ...
                {user.role !== "SCHOOL_ADMIN" && (
                  <Link
                    href={
                      user.role === "TEACHER"
                        ? `/teachers/${user.id}?from=/admin/users`
                        : `/teacher/students/${user.id}?from=/admin/users`
                    }
        ...
        <div className="mt-4 flex flex-wrap items-center justify-end gap-2 text-sm text-slate-200">
          <Link
            href={`/admin/users?page=${Math.max(1, currentPage - 1)}${roleFilter ? `&role=${roleFilter}` : ""}${premiumFilter ? "&premium=true" : ""}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
          ...
          <Link
            href={`/admin/users?page=${Math.min(totalPages, currentPage + 1)}${roleFilter ? `&role=${roleFilter}` : ""}${premiumFilter ? "&premium=true" : ""}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
