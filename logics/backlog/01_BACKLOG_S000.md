...
> Statut : Steps 0→9 livrées (auth/RBAC, positions, blessures, progression, cours, mini-jeu, admin école, navigation unifiée par rôle + espaces dédiés + profil + pagination, Discovery QA). Phase produit enclenchée : facturation livrée (0.6.10) avec invoices auto/backfill, tri/export CSV, toasts ancrés, crédits en résumé, UI admin/prof. Générateur : suggestions visibles sur la fiche cours (teacher/admin), édition des notes via l’écran d’édition (inline retiré). Agendas admin/teacher/student : navigation semaine inline + bouton “Semaine actuelle”. À poursuivre : générateur complet (tags/validation), métriques partenaires, QA parité jeux/agenda élève.
