...
> Statut : Steps 0→9 livrées (auth/RBAC, positions, blessures, progression, cours, mini-jeu, admin école, navigation unifiée par rôle + espaces dédiés + profil + pagination, Discovery QA). Phase produit enclenchée : facturation livrée (0.6.8) avec invoices auto/backfill, tri/export CSV, flashes ancrés, crédits en résumé, UI admin/prof. Générateur : suggestions visibles sur la fiche cours (teacher/admin), édition des notes via l’écran d’édition (inline retiré). À poursuivre : générateur complet (tags/validation), métriques partenaires, agenda admin, QA parité jeux/agenda élève.
