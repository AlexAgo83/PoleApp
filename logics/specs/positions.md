...
- Détail : médias photo (Cloudinary), description, niveau/type, discipline, tips/grips ; accès lecture pour tous les authentifiés ; bouton retour supporte `?fromCourse=` (priorité retour cours/fiche) puis `?from=` (liste/agenda/profil/prof), fallback `/positions`.
